...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.digibyted.height - blockCached.height + 1;
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.digibyted.getBlockHeader(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.digibyted.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.digibyted.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }
      ...
      self.node.services.digibyted.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.digibyted.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.digibyted.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getDGBSubsidy = function getDGBSubsidy(height) {
  var subsidy = 1 * COIN;
...
BlockController.prototype.getBlockValue = function getBlockValue(height) {
  ...
  if (height < 67200) {
      ...
      subsidy = this.getDGBSubsidy(height);
  }
